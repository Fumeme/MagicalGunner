...
public class BrainJuice extends AbstractPotion {
    ...
    public static final String POTION_ID = MagicalBase.makeID("BrainJuice");
    ...
    public void use(AbstractCreature target) {
        ...
        if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) {
            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(target, AbstractDungeon.player, new FocusPower(target, this.potency), this.potency));
            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(target, AbstractDungeon.player, new Mana(target, target, this.potency), this.potency));
        }
    }
    
    @Override
    public AbstractPotion makeCopy() {
        return new BrainJuice();
    }
